 ...
 * Analytics are enabled only in production and when ANALYTICS_ID is set.
 */
export function isAnalyticsEnabled(): boolean {
  return process.env.NODE_ENV === 'production' && !!ANALYTICS_ID;
}

/**
 * Get the Google Analytics (GA4) script URL.
 * Returns empty string if analytics is not enabled.
 */
export function getGA4ScriptUrl(): string {
  if (!isAnalyticsEnabled()) {
    return '';
  }
  return `https://www.googletagmanager.com/gtag/js?id=${ANALYTICS_ID}`;
}
